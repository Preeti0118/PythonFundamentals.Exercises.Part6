...
def convert_to_kelvin(celsius_temp: float) -> float:
    """
        ...
        :param celsiusn_temp: A float representing a temperature in celsius
...
def temperature_tuple(temperatures: Iterable, input_unit_of_measurement: str) -> Tuple[Tuple[float, float]]:
    ...
    """
    list1 = []
    for vartemp in temperatures:
        if input_unit_of_measurement == 'F':
            convtemp = 32 + vartemp * 9/5
        elif input_unit_of_measurement == 'C':
            convtemp = (vartemp -32) * 5/9

            tup1 = (vartemp,convtemp)
            list1.append(tup1)
